...
const InteractivePCB = () => {
  ...
  useEffect(() => {
    ...
    const getTraceColor = () => {
      ...
      return style.getPropertyValue('--pcb-trace').trim() || '255, 107, 0';
    };
    ...
    const render = () => {
      ...
      traces.forEach((trace, index) => {
        ...
        if (isGlowing) {
          ctx.shadowBlur = 15;
          ctx.shadowColor = `rgba(${traceColor}, 1)`;
          ctx.strokeStyle = `rgba(${traceColor}, ${opacity})`;
        } else {
          ctx.shadowBlur = 0;
          ctx.strokeStyle = `rgba(${traceColor}, ${opacity})`;
        }
        ...
        const drawVia = (point) => {
           ...
           ctx.fillStyle = `rgba(${traceColor}, ${opacity})`;
           ctx.fill();
        };
